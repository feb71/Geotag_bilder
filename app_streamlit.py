        ...
        if st.button("Eksporter alle som ZIP (med heading der satt)", key="C_export"):
            ...
            for f in files_up_C:
                try:
                    ...
                    E, N, Alt, hd, cent, line_h, dist = choose_pos_and_heading(
                        picked_label_C, E0, N0, Alt0, None,
...
# --- Streamlit UI snippet: Transfer attributes (put inside a 'with tabE:' block) ---
import streamlit as st
import io, zipfile, json, os
import geopandas as gpd
from pyproj import Transformer

st.header("E) Transfer VA attributes (kilde → mål)")

# CRS picker helper (reuse your pick_epsg if available)
CRS_PRESETS = {"EPSG:25832":25832, "EPSG:5110":5110, "EPSG:4326":4326, "EPSG:25833":25833}
def pick_epsg_widget(label, default=25832, key=None):
    opts = list(CRS_PRESETS.keys()) + ["Custom EPSG"]
    sel = st.selectbox(label, opts, index=0, key=(key or label))
    if sel == "Custom EPSG":
        return int(st.number_input(f"Custom EPSG for {label}", min_value=2000, value=default, step=1, key=(key or label)+"_custom"))
    return CRS_PRESETS[sel]

epsg_choice = pick_epsg_widget("Velg arbeids-CRS (for matching)", default=25832, key="E_epsg_choice")
out_epsg = pick_epsg_widget("Velg CRS for eksport (out)", default=epsg_choice, key="E_out_epsg_choice")

col1, col2 = st.columns([1,1])
with col1:
    uploaded_target = st.file_uploader("Last opp mål-lag (GeoPackage/GeoJSON/Shapefile zip) eller velg fra session", type=["gpkg","geojson","zip","shp"], key="E_up_target")
with col2:
    uploaded_source = st.file_uploader("Last opp kilde-lag (punkt/linje)", type=["gpkg","geojson","zip","shp"], key="E_up_source")

# optional: allow user to use in-memory session shapes (if you have st.session_state["LINES_GDF"] etc.)
# For simplicity, prefer uploaded files for now.

radius_m = st.number_input("Radius (m) for søk", min_value=0.01, value=1.0, step=0.1)
mode = st.selectbox("Mode", ["majority","nearest"], index=0)
angle_weight = st.slider("Angle weight", 0.0, 1.0, 0.2)
z_weight = st.slider("Z weight", 0.0, 1.0, 0.1)
dim_tol = st.number_input("Dim-tolerance (mm)", value=20.0)

# mapping inputs (simple text)
st.markdown("**Felt-mapping** (format: std:field, std2:field2). Standard støttede: system,dim_mm,material")
source_map_txt = st.text_input("Kilde mapping", value="system:system,dim_mm:dim_mm,material:material", key="E_map_src")
target_map_txt = st.text_input("Mål mapping", value="system:system,dim_mm:dim_mm,material:material", key="E_map_tgt")

run = st.button("Kjør transfer", key="E_run_transfer")
if run:
    # Save uploaded files to temp files and load
    def save_uploaded(f):
    ...
    try:
        if uploaded_target is None or uploaded_source is None:
            st.error("Last opp både mål- og kildefil.")
        else:
            tpath = save_uploaded(uploaded_target)
            spath = save_uploaded(uploaded_source)
            st.info("Laster inn lag (kan ta noen sekunder)...")
            gdf_t = gpd.read_file(tpath)
            gdf_s = gpd.read_file(spath)
            st.success(f"Lastet mål: {len(gdf_t)} features; kilde: {len(gdf_s)} entries")
            # call transfer
            out = transfer_va_attributes(
                target_gdf=gdf_t, source_gdf=gdf_s,
                epsg=epsg_choice,
                radius=radius_m,
                match_fields=[m.strip() for m in st.session_state.get("E_match_fields", "system").split(",")],
                source_fields_mapping=source_map_txt,
                target_fields_mapping=target_map_txt,
                mode=mode,
                angle_weight=angle_weight,
                z_weight=z_weight,
                dim_tol=dim_tol,
                allow_assign_missing=True
            )
            st.session_state["E_TRANSFER_OUT"] = out
            st.dataframe(out.head(20))
            # download buttons
            # GeoPackage
            out_bytes = None
            tmpout = tempfile.NamedTemporaryFile(delete=False, suffix=".gpkg")
            try:
                out.to_file(tmpout.name, layer="enriched", driver="GPKG")
                with open(tmpout.name, "rb") as fh:
                    out_bytes = fh.read()
                st.download_button("Last ned GeoPackage (.gpkg)", data=out_bytes, file_name="va_enriched.gpkg")
            except Exception as e:
                st.warning(f"Kunne ikke skrive GPKG: {e}")

            # GeoJSON
            try:
                geojson = out.to_crs(epsg=4326).to_json()
                st.download_button("Last ned GeoJSON", data=geojson.encode("utf-8"), file_name="va_enriched.geojson")
            except Exception as e:
                st.warning(f"Kunne ikke skrive GeoJSON: {e}")
